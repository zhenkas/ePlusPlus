...
#include <memory>
#include <assert.h>
#include <EPP\Preprocessor\pp_debug_only.h>

struct ISafeObject;
typedef std::shared_ptr<ISafeObject> TSafePtr;
typedef std::weak_ptr<ISafeObject> TWeakPtr;

#define assert_pointer(ptr) BOOST_ASSERT(ptr != nullptr);
...
public:
	...
	template <typename T>
	inline static unsigned int GetType(const T & /**/)
	{
		BOOST_STATIC_ASSERT(!std::is_abstract<Creator<T>>::value);
		return 0;
	}
private:
